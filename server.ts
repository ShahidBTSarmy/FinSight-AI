...
// Highly detailed, institutional fallback data for top assets
const mockCompanyDatabase: Record<string, any> = {
  "apple": {
...
// Simple dynamic generator for arbitrary inputs when no GEMINI_API_KEY is supplied
function generateDynamicMockReport(companyName: string) {
  ...
  const investmentScore = 65 + (absHash % 25); // 65 - 90
  ...
  return {
    ...
    economicImpactExplanations: {
      ...
      interestRate: "High capital borrowing costs reduce corporate expansion speed and increase lease servicing requirements.",
      currency: "International sales translate slightly lower with global currency strength variations.",
      oil: "Moderate shipping and freight energy requirements; indirect pricing exposure exists through logistics components.",
      gdp: "Directly sensitive to industrial capital deployment cycles; mild elastic retreat observed when GDP growth compresses."
    },
    keyTakeaways: [
      "Solid mid-tier competitive positioning with highly loyal customer segments.",
      "Prudently controlled overhead structure acts as a defense against margin deterioration.",
      "Sustained re-rating hinges on executing its structural product diversification program."
    ]
  };
}

// 1. COMPANY ANALYZER ENDPOINT (Supports both GET and POST)
app.all('/api/analyze', async (req, res) => {
  const companyName = (req.method === 'POST' ? req.body.companyName : (req.query.q || req.query.companyName)) as string | undefined;
  ...
  // Try mock database first for premium curated outcomes
  if (mockCompanyDatabase[query]) {
    return res.json(mockCompanyDatabase[query]);
  }
  
  // Check if query is close to existing keys
  for (const k of Object.keys(mockCompanyDatabase)) {
    if (query.includes(k) || k.includes(query)) {
      return res.json(mockCompanyDatabase[k]);
    }
  }

  // Attempt real-time Gemini generation
  const client = getGeminiClient();

  if (!client) {
    console.log(`GEMINI_API_KEY is not configured. Falling back to dynamic mock report generator for: ${companyName}`);
    const fallbackReport = generateDynamicMockReport(companyName);
    return res.json({
      ...fallbackReport,
      note: "Using FinSight Local Emulation Engine. Configure your GEMINI_API_KEY in the Secrets panel to activate full real-time AI analyses."
    });
  }

  try {
    const prompt = `Perform a comprehensive, professional investment research analysis for the company: "${companyName}".
You MUST return the output strictly in valid JSON format matching this schema exactly. Do not output any surrounding text or markdown blocks (like \`\`\`json) outside the pure JSON.

JSON Schema structure:
{
  "companyName": "Exact official name of the company (string)",
  "ticker": "Likely ticker symbol, e.g. GOOGL, RELIANCE, TCS (string)",
  "investmentScore": (integer between 0 and 100),
  "scores": {
    "growth": (integer 0-100),
    "financialHealth": (integer 0-100),
    "risk": (integer 0-100 - where higher means more risk),
    "competitiveStrength": (integer 0-100),
    "valuation": (integer 0-100)
  },
  "summary": "A cohesive 2-3 sentence overview of the company's investment thesis, core business, and near-term catalyst. (string)",
  "sentiment": {
    "bullish": (integer percentage, e.g., 65),
    "bearish": (integer percentage, e.g., 15),
    "neutral": (integer percentage, e.g., 20 - sum must equal 100),
    "summary": "A 1-2 sentence institutional news sentiment overview (string)"
  },
  "financials": {
    "revenue": "Brief revenue performance (e.g. '$24.5B (YoY +8.5%)') (string)",
    "profitability": "Gross and Net Profit margins overview (string)",
    "debt": "Debt-to-equity and cash reserve positioning overview (string)"
  },
  "competitors": [
    {
      "name": "Name of competitor 1 (string)",
      "revenue": "Competitor 1 revenue (string)",
      "profitability": "Competitor 1 profitability margin (string)",
      "risk": "Competitor 1 core risk factor (string)",
      "growth": "Competitor 1 growth rate (string)",
      "advantages": ["Competitor advantage 1", "Competitor advantage 2"]
    },
    {
      "name": "Name of competitor 2 (string)",
      "revenue": "Competitor 2 revenue (string)",
      "profitability": "Competitor 2 profitability margin (string)",
      "risk": "Competitor 2 core risk factor (string)",
      "growth": "Competitor 2 growth rate (string)",
      "advantages": ["Competitor advantage 1", "Competitor advantage 2"]
    }
  ],
  "economicImpact": {
    "inflation": (number representing projected % performance impact of a +2% inflation increase, e.g., -3.5),
    "interestRate": (number representing projected % performance impact of a +1.5% interest rate hike, e.g., -4.2),
    "currency": (number representing projected % performance impact of +5% currency strengthening, e.g., 1.5),
    "oil": (number representing projected % performance impact of +20% oil price hike, e.g., -0.8),
    "gdp": (number representing projected % performance impact of +1% GDP growth rate, e.g., 5.4)
  },
  "economicImpactExplanations": {
    "inflation": "Brief explanation of how +2% inflation impacts this company specifically (string)",
    "interestRate": "Brief explanation of how +1.5% interest rates impact this company specifically (string)",
    "currency": "Brief explanation of currency strength or export translation impacts specifically (string)",
    "oil": "Brief explanation of crude or shipping energy price dependency specifically (string)",
    "gdp": "Brief explanation of general economic sensitivity and consumer elasticity specifically (string)"
  },
  "keyTakeaways": [
    "Critical quantitative takeaway 1 (string)",
    "Critical quantitative takeaway 2 (string)",
    "Critical quantitative takeaway 3 (string)"
  ]
}

Rely on your financial intelligence database up to 2026. Be highly realistic, quantitative, professional, and precise. Ensure the JSON is completely valid, clean, and has NO trailing commas.`;
...
// 2. AI INVESTMENT COPILOT (CHAT ENDPOINT)
app.post('/api/chat', async (req, res) => {
  ...
  const getChatFallback = () => {
    ...
    const companyName = companyContext?.companyName || "Tesla";
...
3. *"How does interest rate hikes affect their balance sheet?"*`;
  ...
  try {
    const companyInfo = companyContext ? `We are currently analyzing the company: ${companyContext.companyName} (${companyContext.ticker}).
Context:
- Investment Score: ${companyContext.investmentScore}/100
- Financial Summary: ${companyContext.summary}
- Sentiment: Bullish ${companyContext.sentiment?.bullish}%, Bearish ${companyContext.sentiment?.bearish}%, Neutral ${companyContext.sentiment?.neutral}%
- Revenue/Profitability: ${companyContext.financials?.revenue} | ${companyContext.financials?.profitability}
- Key Takeaways: ${JSON.stringify(companyContext.keyTakeaways)}` : "No specific company is in active focus yet.";

    const systemPrompt = `You are FinSight Copilot, an elite AI Investment Advisor.
Your job is to answer questions about investments, stock analysis, and financial intelligence in clear, simple, professional language that an MBA student, retail investor, or finance analyst would find immediately valuable.

${companyInfo}

Guidelines:
1. Be quantitative and concise.
2. Structure your answers with clear markdown headers, bold terms, and bullet points.
3. Translate complex finance jargon into direct, actionable explanations.
4. Maintain an objective, institutional, premium tone.`;
...
// 3. Integration with Vite (Dev vs Prod mode)
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`FinSight AI Core Server online at http://localhost:${PORT}`);
  });
}
